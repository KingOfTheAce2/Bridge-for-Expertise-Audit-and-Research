...
impl ModelValidator {
    ...
    /// Estimate model quality based on file size and parameters
    pub fn estimate_quality(parameters: &str, file_size: u64) -> ModelQuality {
        let params_value: f64 = parameters
            .trim_end_matches('B')
            .parse()
            .unwrap_or(0.0);
        ...
        // Rough heuristics for quality estimation
        if params_value >= 30.0 {
            ModelQuality::High
        } else if params_value >= 7.0 && size_gb >= 3.0 {
            ModelQuality::Medium
        } else if params_value >= 2.0 {
            ModelQuality::Low
        } else {
            ModelQuality::VeryLow
        }
    }
}
...
mod tests {
    ...
    fn test_estimate_quality() {
        let quality = ModelValidator::estimate_quality("7B", 4_000_000_000);
        assert!(matches!(quality, ModelQuality::Medium));

        let quality = ModelValidator::estimate_quality("1.1B", 700_000_000);
        assert!(matches!(quality, ModelQuality::Low));

        let quality = ModelValidator::estimate_quality("30B", 20_000_000_000);
        assert!(matches!(quality, ModelQuality::High));
    }
}
